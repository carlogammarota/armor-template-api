...
module.exports = (options = {}) => {
  return async (context) => {
    ...
    function generateOrderId() {
      // Generar un número aleatorio entre 0 y 999 (inclusive)
      const randomNum = crypto.randomInt(0, 1000);

      // Formatear el número para que siempre tenga 3 dígitos, rellenando con ceros si es necesario
      const formattedNum = randomNum.toString().padStart(5, "0");

      // Concatenar el prefijo "UL-" con el número formateado
      const orderId = `UL-${formattedNum}`;
